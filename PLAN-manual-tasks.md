...
- Index na `user_id`
- DROP+RECREATE CHECK constraint na executions.task_source (introspekcia existujúceho mena cez pg_constraint) — pridať `'manual'`
...
- Tlačidlo "Run" na každej karte — otvorí výber action (Fix/Feature/Plan) a spustí execution
  ...
  - BE musí validovať ownership: manual task ID patrí autentifikovanému userovi (403 ak nie)
...
6. Negatívny auth test: task usera A spustený userom B cez POST `/executions` (`taskSource: 'manual'`) vráti `403`
