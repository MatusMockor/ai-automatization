...
export function CommandCenter() {
  ...
  const taskExecutions = mockExecutions.filter((e) => e.taskId === selectedTask.id);
    ...
    <div className="flex h-screen bg-background">
            ...
            {filtered.map((task) => (
              <button
                ...
                className={cn(
                  ...
                  selectedTask.id === task.id ? 'bg-primary/5' : 'hover:bg-foreground/[0.02]',
                )}
              >
      ...
      <div className="flex flex-1 flex-col">
        <div className="border-b border-border p-5">
          <span className={cn('mb-2 inline-block rounded px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide', prefixConfig[selectedTask.prefix].activeColor)}>
            {selectedTask.prefix}
          </span>
          <h2 className="mb-2 text-lg font-semibold">{selectedTask.title}</h2>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <ScrollArea className="flex-1">
          <div className="p-5">
            ...
            <p className="mb-6 text-sm leading-relaxed text-foreground/80">{selectedTask.description}</p>

            <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Run with Claude</h3>
            <div className="mb-6">
              <ActionButtons onAction={(action) => console.log(action, selectedTask.id)} />
            </div>

            <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">History</h3>
